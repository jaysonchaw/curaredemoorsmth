...
const AnswerYourself = forwardRef(({ onContinue, onCorrectAnswer, onAnswerResult, isInTransition, isTransitioning, shouldShowNext, questionData, questionIndex, correctAnswersCount = 0, answeredCount = 0, lastThreeQuestions = ['pending', 'pending', 'pending'], onQuit, lessonId = 1, totalQuestions = 9, isPracticeMode = false, practiceProgress = 0, showContentButton = true, isReviewOrSkipQuiz = false, isLightMode = false, isPersonalizedPractice = false }, ref) => {
  ...
  const handleSubmit = () => {
    ...
    // For last 3 questions (indices 6, 7, 8), only 1 attempt - mark as wrong if answer is too short
    // For other questions, always mark as correct (grading happens later)
    if ([6, 7, 8].includes(questionIndex)) {
  ...
  const getAnswerBoxImage = () => {
    const lightSuffix = isLightMode ? '(light)' : ''
    if (questionState === 'correct') {
      return `/ayboxcorrect${lightSuffix}.svg`
    } else if (questionState === 'wrong') {
      return `/ayboxwrong${lightSuffix}.svg`
    }
    return `/ayboxdefault${lightSuffix}.svg`
  }
  ...
  return (
    <div style={{ backgroundColor: isLightMode ? '#ffffff' : '#161d25', minHeight: '100vh', position: 'relative', overflow: 'hidden' }}>
      ...
      >
        {/* Question Text */}
        <div className="flex justify-center px-8" style={{ width: '100%', maxWidth: '800px', marginBottom: '32px' }}>
          <div className="flex items-center" style={{ width: '100%', position: 'relative' }}>
            <h2 className="text-base md:text-lg text-left" style={{ 
              ...
              {questionData?.question || 'Answer the question'}
            </h2>
            {/* Icons container */}
        ...
        {/* Answer Box with newaybox SVG */}
        <div className="flex justify-center px-8" style={{ width: '100%', maxWidth: '800px', position: 'relative' }}>
          <style>
            {`
              .answer-yourself-textarea::placeholder {
                color: ${isLightMode ? '#d0d1d2' : '#3b4652'} !important;
              }
              .answer-yourself-textarea::-webkit-input-placeholder {
                color: ${isLightMode ? '#d0d1d2' : '#3b4652'} !important;
              }
              .answer-yourself-textarea::-moz-placeholder {
                color: ${isLightMode ? '#d0d1d2' : '#3b4652'} !important;
                opacity: 1;
              }
              .answer-yourself-textarea:-ms-input-placeholder {
                color: ${isLightMode ? '#d0d1d2' : '#3b4652'} !important;
              }
            `}
          </style>
          <div style={{ position: 'relative', width: 'calc(200px / 3 * 7)' }}>
            <img
              src={getAnswerBoxImage()}
              alt="Answer Box"
              draggable="false"
            style={{
                width: '100%',
